 ...
 * 1. 프로젝트에 담겨있는 모든 *.jsp 파일을 내부적으로 다시 
 *    *_jsp.java(내부적 servlet 구조와 같다)로
 ...
 * 4. servlet 클래스가 있으면 이 클래스들도 모두 컴파일을 수행하고
 ...
 *
 *@WebServlet(/소문자)
 *	사용자의 request URL(URI) 중 path라는 부분의 ID 값 설정하는 부분
 *  WAS(Web App service, Web App Server)환경에서는
 *  클래스 이름은 외부로 직접 노출되지 않고
 *  path에 지정된 값을 통해서 외부 접근 허용한다
 *  
 *  id값은 현재 프로젝트에서 유일한 값이어야 한다
 *  경우에 따라 /main_path/sub_path/child_path 형식으로
...
public class HelloServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// web에 문자열을 보낼 때 한글(이외의)문자 encoding설정
		response.setContentType("text/html;charset=UTF-8");
		// server에서 web으로 데이터를 return하는 통로 open
		//PrintWriter wOut = response.getWriter();
		//wOut.println(st_name);
		//wOut.close();
		
		// web화면에 어떤 데이터를 표시하는것이 아니고
		// webcontent/hello.jsp파일을 열어서
		// web으로 전송하라
