...
public class ImageService {
	 ...
	 * 기존 방법 : setter 주입방식
	 * 
	 * @Autowired 클래스 객체 사용하는 객체를 final로 선언해서 보호할 수 있고 혹시 모를 해킹에 의한 데이터 변조를 막을 수 있다
	 * 클래스의 교차 참조를 컴파일러 차원에서 방지할 수 있다 setter 주입보다는 constructor(생성자) 주입방식을 사용하자
	 * 
	 * 인텔리제이 intelij setter 주입방식을 사용하면 ide가 심각한 경고를 보인다
	 */
	...
	/*
	 * 일반적으로 dao.insert(VO) 를 호출했을 때 VO에 담아서 전달한 값은 insert()가 수행된 후에 볼 수 있다 seq처럼
	 * SQL에서 생성된 값은 확인 불가능하다 그런데 이 값을 insert() 후에 사용해야할 경우가 있다
	 * 
	 * 이때는 dao, mapper SelectKey를 사용해서 값을 생성하면 insert() 후에 그 값을 사용할 수 있다
	 */
	...
	/*
	 * 혹시 파일이 변경이 되면 기존의 파일 제거 후 변경 된 파일로 대체
	 */
