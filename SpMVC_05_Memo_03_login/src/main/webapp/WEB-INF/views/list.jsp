...
<%
	/*
		css, js 파일을 만들고 jsp, html에서 import를 하여 사용할 경우 해당
		파일을 변경(수정)했을 경우 tomcat이 해당파일이 변경된 것을 
		...
		그래서 대안으로 파일이름을 변경하여 서버를 reloading하는 방법이
		있으나 이 방법또한 계속해서 파일이름을 변경해야하는 불편함이 있다
		
		현재 많은 App에서 사용하는 방법으로
		파일이름 뒤에 dumy query를 부착하여 마치 파일이름을 변경하는 것처럼
		tomcat에게 알린다
		list-table.css파일 뒤에  ?ver-**라는 query를 부착하여
		버전 숫자를 변경하고 tomcat을 reloading해주면 대부분의 문제가 해결된다
		
		따라서 css,js 파일의 코드를 변경한 후에는
		ver 숫자를 지금과 다른 값으로 변경하고 tomcat을 reloading하자
			
	*/
%>
	<link rel="stylesheet" type="text/css"
		href="${rootPath}/css/list-table.css?ver=20191202">
	<table>
		<tr>
			<th>SQ</th>
		...
		<c:choose>
			...
			<c:otherwise>
				<c:forEach items="${MEMO_LIST}" var="memo" varStatus="index">
					<tr class="context-body" data-id="${memo.m_seq}"
